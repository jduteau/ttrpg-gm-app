...
export default function ChatWindow({ session, campaign, onSessionTitleChange, onOpenSidebar }) {
  ...
  const handleEndSession = async () => {
    setShowEndConfirm(false);
    setEnding(true); setStateBuffer(''); setReportBuffer('');

    try {
      const res = await fetch(apiUrl(`/api/sessions/${session.id}/end`), { 
        method: 'POST',
        headers: getAuthHeaders()
      });
        ...
        try {
          ...
          if (data.done) {
            if (isEnd) {
              ...
              const fresh = await fetch(apiUrl(`/api/sessions/${session.id}/messages`), {
                headers: getAuthHeaders()
              }).then(r => r.json());
